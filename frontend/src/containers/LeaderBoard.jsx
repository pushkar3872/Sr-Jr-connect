...
export default function LeaderBoard() {
  ...
  const getUserRank = (user) => {
    if (selectedPlatform === 'leetcode') {
      ...
      return user.Competitive_Programming?.CodeforcesData?.rank;
    }
  };
      ...
      >
        {isLoading ? (
          ...
          displayUsers.map((leader, index) => (
            <div
              key={leader._id}
              className={`relative flex gap-2 justify-between items-center p-4 rounded-xl transition-all duration-300 shadow-sm hover:shadow-md ${index === 0
